...
import { calcRentDaysWithGrace } from "@/lib/rent-days"
...
export function useInformationStepLogic() {
  ...
  const dt = searchParams.get("dt") || "10:00"
  const rt = searchParams.get("rt") || "10:00"
  ...
  function calcTotals(args: {
    ...
    try {
      ...
      rentDays = parseInt(apiData.item.rent_days) || 1
    }
    ...
    if (Array.isArray(apiData.options)) {
      ...
      selectedOptions.forEach((optId) => {
        ...
        const preOpt = parseFloat(opt.pre_price_pay || "0")
        prePayPrice += preOpt
    ...
    if (Array.isArray(apiData.places)) {
      ...
      const getPlaceById = (id: any) => places.find((p) => p && String(p.id) === String(id))
      ...
      if (deliveryLocation?.location) {
        if (deliveryLocation.location === "desired") {
          ...
          const delPrice = parseFloat((del as any)?.price_pay || "0")
          const delPre = parseFloat((del as any)?.pre_price_pay || "0")
      ...
      if (effectiveReturn?.location) {
        if (effectiveReturn.location === "desired") {
          ...
          const retPrice = parseFloat((ret as any)?.price_pay || "0")
          const retPre = parseFloat((ret as any)?.pre_price_pay || "0")
