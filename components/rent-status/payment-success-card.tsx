...
// ── shake keyframes (same as LoginDialog) ─────────────────────────────────
const SHAKE_STYLE = `
@keyframes otp-shake {
...
type UploadedDoc = { id?: number; status?: string; file_path?: string | null; file_url?: string | null; rejection_reason?: string | null; } | null;
...
function logFormData(fd: FormData) {
  ...
  for (const [key, value] of fd.entries()) {
    if (value instanceof File) console.log(key, { name: value.name, type: value.type, size: value.size });
...
// ══════════════════════════════════════════════════════════════════════════
// OtpLockScreen — styled exactly like LoginDialog / screenshot
// ══════════════════════════════════════════════════════════════════════════
function OtpLockScreen({
  ...
  useEffect(() => {
    setOtp(""); setLoading(false); setCooldown(0);
    setStep("send"); setOtpError(false); setSmsError(null);
  }, [fixedMobile]);
  ...
  const sendCode = async () => {
    ...
    setLoading(true); setSmsError(null);
    try {
      await otpRequest(fixedMobile);
      setCooldown(75); setStep("verify"); setOtp("");
    } catch (e: any) {
      setSmsError(e?.message ?? "خطا در ارسال کد");
  ...
  const resend = async () => {
    ...
    try {
      await otpRequest(fixedMobile);
      setCooldown(75); setOtp(""); setStep("verify"); setOtpError(false);
    } catch (e: any) {
      setSmsError(e?.message ?? "ارسال مجدد ناموفق بود");
    } finally { setLoading(false); }
  };
  ...
  const verify = async (code: string) => {
    ...
    setLoading(true);
    try {
      const res = await signIn("otp", { redirect: false, mobile: fixedMobile, code });
  ...
  // ── render (full-screen mobile / centered card on sm+, like LoginDialog) ──
  return (
    <>
      <InjectShakeStyle />

      {/*
        Mobile:  full-screen white (inset-0, h-dvh, flex-col)
        sm+:     centered card (max-w-4xl, rounded-2xl, border, shadow)
        — mirrors LoginDialog DialogContent exactly —
      */}
      {/*
        Mobile:  full-screen white
        sm+:     backdrop + centered card, max-w-lg (smaller than LoginDialog's max-w-4xl)
      */}
        ...
        >
          ...
          <div className="flex items-center px-5 pt-5">
            {step === "verify" && (
              <button
                type="button"
                onClick={() => { setStep("send"); setOtp(""); setOtpError(false); setSmsError(null); }}
                aria-label="بازگشت"
          ...
          <div className="flex-1 overflow-y-auto px-6 sm:px-10">
            ...
            {step === "send" && (
              <div dir="rtl">
                <h2 className="mb-6 text-center text-[20px] font-bold text-gray-900">
                  برای تکمیل فرآیند، ابتدا وارد شوید
                </h2>

                {/* Info banner */}
                <div className="mb-4 rounded-xl border border-[#bae6fd] bg-[#eff8ff] px-2 py-3 text-center text-sm leading-6 text-[#0369a1]">
                  شماره تلفن شما قبلاً در سامانه ثبت‌نام شده است.
                  <br />
                  لطفاً برای ادامه فرآیند رزرو، وارد حساب کاربری خود شوید.
                </div>
                ...
                {/*
                  PhoneInputCustom — same component as LoginDialog, locked to Iran.
                  read-only + lockedCountry so user can't change flag or number.
                */}
                ...
                {smsError && (
                  <p className="mb-2 px-1 text-xs text-red-500" dir="rtl">{smsError}</p>
                )}

                {sessionLoading && (
                  <p className="mt-3 text-center text-xs text-gray-400">در حال بررسی وضعیت ورود...</p>
                )}

                <p className="mt-4 text-center text-xs leading-5 text-gray-400">
                  با کلیک بر روی دکمه، کد تایید برای شما ارسال می‌شود.
                </p>
              </div>
            )}
            ...
            {step === "verify" && (
              <div dir="rtl">
                ...
                {/* Phone info row — identical to LoginDialog OTP step */}
                ...
                {/* OTP slots — identical classes to LoginDialog */}
                ...
                {otpError && (
                  <p className="mb-2 text-center text-xs text-red-500">کد وارد شده صحیح نیست</p>
                )}

                {/* Cooldown / resend — identical to LoginDialog */}
                ...
                {smsError && (
                  <p className="mt-1 text-center text-xs text-red-500" dir="rtl">{smsError}</p>
                )}
              </div>
            )}
          </div>

          {/* ── Footer — identical classes to LoginDialog ── */}
  ...
  return (
    <div className="relative flex items-center justify-center">
      <svg width={size} height={size} className="block">
        ...
        <circle cx={size / 2} cy={size / 2} r={r} strokeWidth={stroke} className="text-green-600" stroke="currentColor" fill="none"
          strokeLinecap="round"
...
// ══════════════════════════════════════════════════════════════════════════
function UploadThanksLockedDialog({ open, secondsLeft, onGoHome }: { open: boolean; secondsLeft: number; onGoHome: () => void; }) {
  if (!open) return null;
  return (
        ...
        <div className="space-y-6 p-7 text-center sm:p-9">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-7 text-gray-600 dark:text-gray-300">
              ممنون 🙏 مدارک شما دریافت شد و در حال بررسی است.<br />
          ...
          <div className="rounded-2xl border border-gray-100 bg-gray-50/70 p-4 text-right dark:border-gray-800 dark:bg-gray-800/30">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs leading-6 text-gray-700 dark:text-gray-200">
                <div className="mb-1 font-black">پشتیبانی</div>
                اگر عکس‌ها خوانا نبود یا مشکلی داشت (نور، برش، تاری و ...)، با پشتیبانی هماهنگ کنید تا سریع‌تر بررسی شود.
              </div>
            </div>
          </div>
          <Button type="button" onClick={onGoHome} className="h-12 w-full gap-2 rounded-2xl font-extrabold">
...
// UploadTile
// ══════════════════════════════════════════════════════════════════════════
function UploadTile({ label, file, serverUrl, onPick, onRemove, hasError, gridOneSlot, disabled }: {
  label: string; file?: File | null; serverUrl?: string | null;
  onPick: () => void; onRemove?: () => void; hasError?: boolean; gridOneSlot?: boolean; disabled?: boolean;
}) {
  const [preview, setPreview] = useState<string | null>(null);

  useEffect(() => {
    if (file) { const url = URL.createObjectURL(file); setPreview(url); return () => URL.revokeObjectURL(url); }
    if (serverUrl) { setPreview(serverUrl); return; }
    setPreview(null);
  }, [file, serverUrl]);
  ...
  return (
    <div className={gridOneSlot ? "col-span-1" : "w-full"}>
      <div className="relative">
        <button
          type="button"
          onClick={() => { if (disabled) return; onPick(); }}
          className={cn(
        ...
        >
          {preview && <Image src={preview} alt={label} fill className="object-cover" sizes="(max-width: 640px) 50vw, 240px" />}
      ...
      {hasError && <div className="mt-1 text-center text-[11px] text-red-600">این مورد الزامی است</div>}
    </div>
  );
}
...
export function PaymentSuccessCard({ rentData, trace, onDownloadVoucher, onSubmitUpload }: any) {
  ...
  const readonlyPhoneRaw = String(rentData?.auth?.phone ?? rentData?.rent_info?.phone ?? rentData?.summary?.phone ?? rentData?.phone ?? "") || "";
  ...
  const carTitle = useMemo(() => [car?.brand, car?.model, car?.year].filter(Boolean).join(" "), [car?.brand, car?.model, car?.year]);
  ...
  const remainAtDelivery = details?.totals?.remain_to_pay ?? payment?.remain_to_pay ?? payment?.remaining ?? payment?.remain ?? rentData?.remain_to_pay;
  ...
  const onFile = (k: UploadKey, f: File | null) => {
    if (shouldHardLock) { toast.info("برای ادامه باید وارد شوید"); return; }
    setFiles((p) => ({ ...p, [k]: f }));
  ...
  useEffect(() => {
    ...
    setSecondsLeft(5);
    let mounted = true;
    const interval = setInterval(() => { if (!mounted) return; setSecondsLeft((s) => s - 1); }, 1000);
    const timeout = setTimeout(() => { if (!mounted) return; router.push("/"); }, 5000);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setServerDocs(next);
        if (!Boolean(next.intl_dl_front?.file_url) && !Boolean(next.intl_dl_back?.file_url)) setNoIntl(false);
        if (!Boolean(next.visa?.file_url)) setNoVisa(false);
  ...
  const validate = () => {
    if (shouldHardLock) { toast.info("برای آپلود مدارک باید وارد شوید"); return false; }
    ...
    if (!noVisa && !files.visa && !hasServer("visa")) next.visa = true;
    setFieldErrors(next);
    if (Object.values(next).some(Boolean)) { toast.warning("لطفاً مدارک الزامی را تکمیل کنید"); return false; }
    return true;
  };

  const submit = async () => {
    try {
      if (shouldHardLock) { toast.info("برای آپلود مدارک باید وارد شوید"); return; }
      setErr(null);
      if (!validate()) return;
      setLoading(true);
      ...
      if (!noVisa && files.visa) fd.append("visa_file", files.visa);
      const hasAnyNewFile = [...fd.keys()].length > 0;
      if (!hasAnyNewFile) { setOpenThanks(true); return; }
      ...
      logFormData(fd);
      if (typeof onSubmitUpload === "function") await onSubmitUpload(fd);
      else await uploadUserDocumentsFormData(fd);
      try {
        const data = await getUserDocuments();
        const next: ServerDocsState = {};
        (Object.keys(MAP) as UploadKey[]).forEach((k) => { const [type, side] = MAP[k]; next[k] = data?.[type]?.[side] ?? null; });
        setServerDocs(next); setFiles({});
      } catch { setFiles({}); }
  ...
  const handleDownloadVoucher = async () => {
    ...
    if (!rentId) { toast.error("rent_id پیدا نشد"); return; }
    if (typeof onDownloadVoucher === "function") { onDownloadVoucher(); return; }
    try {
      setDownloadingVoucher(true);
      toast.info("در حال آماده‌سازی وچر...");
      const res = await fetch(`/api/voucher-pdf?rent_id=${rentId}`, { method: "GET", credentials: "include" });
      if (!res.ok) { const txt = await res.text().catch(() => ""); throw new Error(txt || `PDF API failed: ${res.status}`); }
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `voucher-${rentId}.pdf`;
      document.body.appendChild(a); a.click(); a.remove();
      window.URL.revokeObjectURL(url);
      toast.success("وچر دانلود شد");
    } catch (e: any) { console.error(e); toast.error(e?.message || "دانلود وچر ناموفق بود"); }
    finally { setDownloadingVoucher(false); }
  };
  ...
  const invoiceRows = useMemo(() => {
    ...
    for (const o of optItems) {
      ...
      rows.push({ label: `${String(o?.title ?? "آپشن")} × ${toFaNumber(num)}`, value: formatMoneyOrFree(sumPrice, currency) });
    }
    ...
    if (typeof prePay !== "undefined" && prePay !== null) rows.push({ label: "پیش پرداخت", value: `${formatMoneyFa(prePay, currency)} (پرداخت شده)` });
    if (typeof remainAtDelivery !== "undefined" && remainAtDelivery !== null) rows.push({ label: "مانده حساب", value: formatMoneyFa(remainAtDelivery, currency) });
    if (typeof sumAll !== "undefined" && sumAll !== null) rows.push({ label: `هزینه نهایی برای ${toFaNumber(days || summary?.days || 0)} روز`, value: formatMoneyFa(sumAll, currency) });
    return rows;
  }, [details, currency, days, summary?.days, remainAtDelivery, prePay, sumAll]);

  // ── Hard lock: show OtpLockScreen ──────────────────────────────────────
  if (shouldHardLock) {
    return <OtpLockScreen mobile={readonlyPhone} sessionLoading={sessionLoading} />;
  }

  // ── Payment success page ───────────────────────────────────────────────
  return (
    <div className="w-full bg-white dark:bg-gray-900">
      <UploadThanksLockedDialog open={openThanks} secondsLeft={Math.max(0, secondsLeft)} onGoHome={() => router.push("/")} />
      ...
      <div className="mx-auto w-full max-w-5xl px-4 pb-24">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <button
            ...
            className={cn("inline-flex items-center gap-2 font-black text-blue-600", downloadingVoucher && "cursor-not-allowed opacity-60")}
          >
        ...
        <Card className="mt-2 rounded-2xl border border-gray-200 shadow-none dark:border-gray-800">
          <CardContent className="p-0">
            <button type="button" onClick={() => setOpenInfo((p) => !p)}
              className="flex w-full items-center justify-between p-3" aria-label="toggle info"
            >
              <div className="text-right">
                <div className="font-black text-gray-900 dark:text-white">
                  {carTitle || summary?.car_name || "—"}
                  {days ? <span className="ml-1 font-bold text-gray-500">({toFaNumber(days)} روز)</span> : null}
                </div>
                <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">از {fromText} تا {toText}</div>
              ...
              <ChevronDown className={cn("h-5 w-5 text-gray-500 transition-transform", openInfo ? "rotate-180" : "rotate-0")} />
            </button>
        ...
        <div className="mt-5">
          ...
          <div className="mt-3 grid grid-cols-2 gap-4">
            <UploadTile label="عکس" file={files.id_card} serverUrl={serverDocs.id_card?.file_url ?? null}
              onPick={() => inputs.current.id_card?.click()} onRemove={() => removeFile("id_card")}
              hasError={fieldErrors.id_card} gridOneSlot />
            <div className="hidden" />
          </div>
          <Input ref={(el) => { inputs.current.id_card = el as unknown as HTMLInputElement; }}
        ...
        <div className="mt-6">
          ...
          <div className="mt-3 grid grid-cols-2 gap-4">
            <div>
              <UploadTile label="سمت عقب" file={files.dl_back} serverUrl={serverDocs.dl_back?.file_url ?? null}
                onPick={() => inputs.current.dl_back?.click()} onRemove={() => removeFile("dl_back")}
                hasError={fieldErrors.dl_back} />
              <Input ref={(el) => { inputs.current.dl_back = el as unknown as HTMLInputElement; }}
                type="file" accept="image/*" className="hidden"
                onChange={(e) => onFile("dl_back", (e.target as HTMLInputElement).files?.[0] ?? null)} />
            </div>
            <div>
              <UploadTile label="جبهه" file={files.dl_front} serverUrl={serverDocs.dl_front?.file_url ?? null}
                onPick={() => inputs.current.dl_front?.click()} onRemove={() => removeFile("dl_front")}
                hasError={fieldErrors.dl_front} />
              <Input ref={(el) => { inputs.current.dl_front = el as unknown as HTMLInputElement; }}
                type="file" accept="image/*" className="hidden"
        ...
        {/* International DL */}
        <div className="mt-6">
          <div className="border-b text-lg font-black text-gray-900 dark:text-white">گواهینامه رانندگی بین المللی</div>
          <div className="mt-2 flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-300">گواهینامه رانندگی بین المللی ندارم</div>
            <Checkbox checked={noIntl} onCheckedChange={(checked) => {
              const v = checked === true; setNoIntl(v);
          ...
          {!noIntl && (
            <div className="mt-3 grid grid-cols-2 gap-4">
              <div>
                <UploadTile label="سمت عقب" file={files.intl_dl_back} serverUrl={serverDocs.intl_dl_back?.file_url ?? null}
                  onPick={() => inputs.current.intl_dl_back?.click()} onRemove={() => removeFile("intl_dl_back")}
                  hasError={fieldErrors.intl_dl_back} />
                <Input ref={(el) => { inputs.current.intl_dl_back = el as unknown as HTMLInputElement; }}
                  type="file" accept="image/*" className="hidden"
                  onChange={(e) => onFile("intl_dl_back", (e.target as HTMLInputElement).files?.[0] ?? null)} />
              </div>
              <div>
                <UploadTile label="جبهه" file={files.intl_dl_front} serverUrl={serverDocs.intl_dl_front?.file_url ?? null}
                  onPick={() => inputs.current.intl_dl_front?.click()} onRemove={() => removeFile("intl_dl_front")}
                  hasError={fieldErrors.intl_dl_front} />
                <Input ref={(el) => { inputs.current.intl_dl_front = el as unknown as HTMLInputElement; }}
                  type="file" accept="image/*" className="hidden"
        ...
        {/* Visa */}
        <div className="mt-6">
          <div className="border-b text-lg font-black text-gray-900 dark:text-white">ویزا</div>
          <div className="mt-2 flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-300">من ویزا ندارم</div>
            <Checkbox checked={noVisa} onCheckedChange={(checked) => {
              const v = checked === true; setNoVisa(v);
          ...
          {!noVisa && (
            <>
              <div className="mt-3 grid grid-cols-2 gap-4">
                <UploadTile label="عکس" file={files.visa} serverUrl={serverDocs.visa?.file_url ?? null}
                  onPick={() => inputs.current.visa?.click()} onRemove={() => removeFile("visa")}
                  hasError={fieldErrors.visa} gridOneSlot />
                <div className="hidden" />
              </div>
              <Input ref={(el) => { inputs.current.visa = el as unknown as HTMLInputElement; }}
                type="file" accept="image/*" className="hidden"
