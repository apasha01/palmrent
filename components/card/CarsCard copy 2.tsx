    ...
    >
      ...
      <div className="pl-2.5 flex flex-col">
        <div className="flex items-center justify-between">
          <span className="size-6 text-[#333333]" onClick={(e) => e.stopPropagation()}>
            <IconHeart active={undefined} />
          </span>
          <div className="text-left my-2 text-lg font-bold">
            {locale === "fa" ? toFaDigits(capitalizeWords((car as any).title)) : capitalizeWords((car as any).title)}
          </div>
        </div>
  ...
  return (
    <div className={`grid grid-cols-4 gap-1 text-[#787878] mt-1 mb-4 ${textSize} border-y p-2 text-nowrap`}>
      ...
      <div className="flex items-center justify-center gap-1">
        ...
        <span className="text-xs">{toFaDigits(car.baggage ?? car.suitcase ?? 0) || 0} {t("suitCase")}</span>
      </div>
      <div className="flex items-center justify-center gap-1">
        <span className={bigFont ? "xl:size-5 size-4" : "size-4"}><IconPerson /></span>
        <span className="text-xs">{toFaDigits(car.passengers ?? 0) || 0} {t("people")}</span>
      </div>
    </div>
  )
}
...
export function SingleCarPriceList({
  priceList, defaultPrice, oldPrice, carDates, deliveryTime, returnTime, currency,
}: {
  priceList: any; defaultPrice?: number | null; oldPrice?: number | null
  carDates: [string | null, string | null] | null; deliveryTime: string | null
  returnTime: string | null; currency: string; rateToRial?: number | null
}) {
  const t = useTranslations()
  const locale = useLocale()
  ...
  const daily = useMemo(() => {
    ...
    const list = Array.isArray(priceList) ? priceList : priceList && typeof priceList === "object" ? Object.values(priceList) : []
  ...
  const dailyOld = useMemo(() => { const v = Number(oldPrice ?? 0); return v > 0 ? v : 0 }, [oldPrice])
...
export function SingleCarButtons({
  car, carDates, deliveryTime, returnTime, reserveOnClick,
}: {
  car: any; carDates: [string | null, string | null] | null
  deliveryTime: string | null; returnTime: string | null; reserveOnClick: () => void
}) {
  const t = useTranslations()
  const locale = useLocale()
  const loc = useMemo(() => String(locale || "en").toLowerCase().split("-")[0], [locale])
  const carId = useMemo(() => { const v = car?.id ?? car?.car_id ?? 0; const n = Number(v); return Number.isFinite(n) ? n : 0 }, [car])
  ...
  const days = useMemo(() => {
    ...
    try { return calcDaysWithGraceSafe({ carDates, deliveryTime, returnTime }) } catch { return 0 }
  ...
  const whatsappText = useMemo(() => {
    ...
    return t("whatsappMessage.reserveWithDate", { car: carTitle, city, url: carUrl, from, to, dt, rt, days: String(days || "") })
