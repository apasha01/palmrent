...
/** Format date based on locale — Jalali ONLY for fa, Gregorian for everything else */
function formatLocalizedDate(date: Date | null, locale: string): string {
  if (!date) return "---";

  // ✅ فقط فارسی شمسی میگیره — عربی و بقیه همه میلادی
  if (locale === "fa") {
      ...
      <div className="flex items-center h-12 w-full overflow-hidden bg-transparent">
        <div className="flex items-center gap-1.5 px-2 w-full min-w-0">
          ...
          <div className="min-w-0 flex-1 overflow-hidden">
            {hasDate ? (
              <p
                className={`truncate ${dateTextSize} text-gray-900 dark:text-gray-100 font-medium`}
              >
                {formattedDate}
                <span
                  className={`${dateTextSize} text-gray-900 dark:text-gray-100 font-medium`}
                >
                  {" - "}
...
export default NavSectionSearchClient;
