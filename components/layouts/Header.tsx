...
export function HeaderMenu({ closeMenu, locale }: HeaderMenuProps) {
  ...
  function toggleMenu(targetIndex: number) {
    setDropMenuToggle(dropMenuToggle.map((item, index) => (index !== targetIndex ? false : !item)));
  }
          ...
          {!isLoading &&
            ...
            sortedBranches.map((b: any) => (
              <DropDownItem
                key={b.id}
                text={b.title}
                // ✅✅✅ App Router-safe: href باید مسیر نهایی باشد
  ...
  return (
    <li className="pb-2 px-2">
      <div className="flex items-center justify-between h-11 px-2 rounded-lg ">
        ...
        <Switch dir="ltr" checked={!!isDark} onCheckedChange={toggleTheme} disabled={!mounted} />
      </div>
    </li>
  );
}
