...
import { getAboutUsPage } from "@/services/settings/setting-service.api";
...
export async function generateMetadata({
  ...
  const response = await getAboutUsPage(locale);

  if (response.status === 404) {
    return {
      title: "About Us Not Found | Palm Rent",
  ...
  if (response.status !== 200) {
    return {
      title: "About Us | Palm Rent",
      description: "About Us | Palm Rent",
      robots: "noindex, nofollow",
    };
  }
  ...
  return {
    title: meta?.titleSeo || "About Us | Palm Rent",
    description: meta?.descriptionSeo || "About Us | Palm Rent",
...
export default async function AboutUsPage({ params }: PageProps) {
  ...
  if (response.status !== 200) {
    throw new Error(`Failed to load about us page. Status: ${response.status}`);
  }
