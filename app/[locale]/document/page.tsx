...
import { getDocumentPage } from "@/services/settings/setting-service.api";
...
export async function generateMetadata({
  ...
  const response = await getDocumentPage(locale);

  if (response.status === 404) {
    return {
      title: "Document Not Found | Palm Rent",
  ...
  if (response.status !== 200) {
    return {
      title: "Document | Palm Rent",
      description: "Document | Palm Rent",
      robots: "noindex, nofollow",
    };
  }
  ...
  return {
    title: meta?.titleSeo || "Document | Palm Rent",
    description: meta?.descriptionSeo || "Document | Palm Rent",
...
export default async function DocumentPage({ params }: PageProps) {
  ...
  if (response.status !== 200) {
    throw new Error(`Failed to load document page. Status: ${response.status}`);
  }
