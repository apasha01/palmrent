...
export default function DownloadAppPageUI({
  ...
  const resolvedAppName = appName || t("appName");
  const resolvedSubtitle = subtitle || t("subtitle");

  return (
    <div dir={dir} className="relative w-full overflow-hidden bg-background">
      <Header />
      ...
      <main className="relative mx-auto w-full max-w-6xl px-4 py-10 md:py-14">
        <div className="flex items-center justify-between gap-3">
          <div className="inline-flex flex-wrap items-center gap-2">
            ...
            >
              <Sparkles className={`${isRtl ? "me-1" : "mr-1"} h-4 w-4`} />
              {t("badges.newVersion")}
            </Badge>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <h1 className="text-2xl font-bold leading-tight md:text-4xl">
                  {t("hero.downloadTitle", { appName: resolvedAppName })}
                </h1>
              </div>
            </div>
            ...
            <div className="rounded-2xl border bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-emerald-500/10 p-4 backdrop-blur">
              <div className="flex flex-wrap items-center gap-2 text-sm">
                ...
                <span className="inline-flex items-center gap-2 text-muted-foreground">
                  ...
                  {t("info.orderStatusNotification")}
                </span>
              ...
              <Card className="group rounded-2xl border bg-white/60 backdrop-blur transition-all hover:-translate-y-1 hover:shadow-xl dark:bg-white/5">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 text-sm font-semibold">
                    ...
                    {t("features.easyInstall.title")}
                  </div>
                  <p className="mt-2 text-xs leading-6 text-muted-foreground">
                    {t("features.easyInstall.description")}
                  </p>
              ...
              <Card className="group rounded-2xl border bg-white/60 backdrop-blur transition-all hover:-translate-y-1 hover:shadow-xl dark:bg-white/5">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 text-sm font-semibold">
                    ...
                    {t("features.betterExperience.title")}
                  </div>
                  <p className="mt-2 text-xs leading-6 text-muted-foreground">
                    {t("features.betterExperience.description")}
                  </p>
            ...
            <div className="mt-2 flex flex-col gap-3 sm:flex-row">
              <Button
                asChild
                size="lg"
                className="h-12 gap-2 rounded-2xl shadow-md transition-transform hover:-translate-y-0.5 hover:shadow-xl"
              >
                <a href={androidHref} className="relative overflow-hidden">
              ...
              <Button
                ...
                size="lg"
                variant="secondary"
                className="h-12 gap-2 rounded-2xl shadow-sm transition-transform hover:-translate-y-0.5 hover:shadow-lg"
              >
                <a href={iosHref}>
                  <Download className="h-4 w-4" />
                  {t("actions.downloadIOS")}
                </a>
              </Button>
              ...
              <Button
                ...
                size="lg"
                variant="outline"
                className="h-12 gap-2 rounded-2xl transition-transform hover:-translate-y-0.5 hover:bg-white/50 dark:hover:bg-white/10"
              >
                <a href={webHref}>
                  <Globe className="h-4 w-4" />
                  {t("actions.webVersion")}
                  <ArrowUpLeft className="h-4 w-4" />
                </a>
              </Button>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>{t("support.installProblem")}</span>
              <Link href={helpHref} className="underline underline-offset-4">
                {t("support.installationGuide")}
              </Link>
            </div>
          </div>
            ...
            <div className="mx-auto w-full max-w-md">
              ...
              <div className="relative rounded-[2.8rem] border bg-card/70 p-3 shadow-2xl backdrop-blur">
                ...
                <div className="absolute -left-2 top-10 hidden -rotate-6 sm:block">
                  <div className="animate-float rounded-2xl border bg-background/80 px-3 py-2 shadow-md backdrop-blur">
                    <p className="text-xs font-semibold">
                      {t("floating.quickBooking")}
                    </p>
                    <p className="text-[11px] text-muted-foreground">
                      {t("floating.lessThanOneMinute")}
                    </p>
                  </div>
                </div>

                <div className="absolute -right-2 bottom-10 hidden rotate-6 sm:block">
                  <div className="animate-float rounded-2xl border bg-background/80 px-3 py-2 shadow-md backdrop-blur animation-delay-1200">
                    <p className="text-xs font-semibold">
                      {t("floating.support247")}
                    </p>
                    <p className="text-[11px] text-muted-foreground">
                      {t("floating.whatsappAndCall")}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 sm:hidden">
                <Link href={webHref} className="block">
                  <Button
                    variant="outline"
                    size="lg"
                    className="h-12 w-full gap-2 rounded-2xl hover:bg-white/50 dark:hover:bg-white/10"
                  >
                    <Globe className="h-4 w-4" />
                    {t("actions.enterWeb")}
                    <ArrowUpLeft className="h-4 w-4" />
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="grid w-full gap-3 md:w-auto md:grid-cols-3">
                  <Button
                    asChild
                    variant="outline"
                    className="h-12 gap-2 rounded-2xl hover:bg-white/50 dark:hover:bg-white/10"
                  >
                    <a href={androidHref}>
                      <Download className="h-4 w-4" />
                      {t("bottom.android")}
