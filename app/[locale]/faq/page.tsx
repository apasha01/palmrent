...
import { getFaqPage } from "@/services/settings/setting-service.api";
...
export async function generateMetadata({
  ...
  const response = await getFaqPage(locale);

  if (response.status === 404) {
    return {
      title: "FAQ Not Found | Palm Rent",
  ...
  return {
    title: meta?.titleSeo || "FAQ | Palm Rent",
    description: meta?.descriptionSeo || "FAQ | Palm Rent",
...
export default async function FaqPage({ params }: PageProps) {
  ...
  if (response.status !== 200) {
    throw new Error(`Failed to load FAQ page. Status: ${response.status}`);
  }
